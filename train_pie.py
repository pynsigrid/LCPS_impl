...
from dataloader.dataset import Nuscenes_pt, collate_fn_BEV, spherical_dataset, collate_dataset_info, SemKITTI_pt
from dataloader.dataset_pie import Nuscenes_pt_ial, spherical_dataset_pie
...
def main():
    ...
    if datasetname == 'SemanticKitti':
        ...
        train_pt_dataset = Nuscenes_pt_ial(data_path, split='train', cfgs=cfgs, nusc=nusc, version=version)
        val_pt_dataset = Nuscenes_pt(data_path, split='val', cfgs=cfgs, nusc=nusc, version=version)
    else:
        raise NotImplementedError

    train_dataset = spherical_dataset_pie(train_pt_dataset, cfgs, ignore_label=0)
