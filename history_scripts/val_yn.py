...
def main():
    ...
    with torch.no_grad():
        for i_iter_val, val_dict in enumerate(val_dataset_loader):
            ...
            for count, i_val_grid in enumerate(val_grid):
                ...
                panoptic_labels, center_points = get_panoptic_segmentation(
                    ...
                    threshold=cfgs['model']['post_proc']['threshold'],
                    nms_kernel=cfgs['model']['post_proc']['nms_kernel'],
                    top_k=cfgs['model']['post_proc']['top_k'], 
                    polar=True, 
        ...
        if args.local_rank < 1:
            if args.local_rank == 0:
                ...
                for i in range(world_size - 1):
                    ...
                    for j in range(len(cur_dict['item1'])):
                        ...
                        evaluator.addBatch(cur_dict['item1'][j], cur_dict['item2'][j], 
                                         sem_gt, inst_gt)
            ...
            PQ_th = np.nanmean(class_all_PQ[1: upper_idx + 1])
            SQ_th = np.nanmean(class_all_SQ[1: upper_idx + 1])
            RQ_th = np.nanmean(class_all_RQ[1: upper_idx + 1])
            PQ_st = np.nanmean(class_all_PQ[upper_idx+1: -1])
            SQ_st = np.nanmean(class_all_SQ[upper_idx+1: -1])
            RQ_st = np.nanmean(class_all_RQ[upper_idx+1: -1])

            # 输出总体指标
            logger.info('PQ %.1f  PQ_dagger  %.1f  SQ %.1f  RQ %.1f  |  PQ_th %.1f  SQ_th %.1f  RQ_th %.1f  |  PQ_st %.1f  SQ_st %.1f  RQ_st %.1f  |  mIoU %.1f' % (
